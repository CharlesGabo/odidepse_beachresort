  ...
  return <section className="bi-page" aria-labelledby="analytics-title">
    ...
    {report && <div className={stale ? 'bi-results is-stale' : 'bi-results'} aria-busy={busy}>
      ...
      <div className="bi-kpis">{[
        ...
        ['Cancellation rate',m.cancellation_rate === null ? '—' : `${number(m.cancellation_rate)}%`,`${number(m.cancelled_requests)} cancelled ${Number(m.cancelled_requests) === 1 ? 'booking' : 'bookings'} of ${number(m.requests)} requests`, 'cancelled'],
      ...
      <div className="bi-chart-grid">
        ...
        <Trend title="Financial performance" caption="PHP · check-in value compared with cash by transaction date" rows={report.series} currency latestValues lines={[{key:'agreed',label:'Agreed booking value',endLabel:'Agreed'},{key:'net',label:'Net cash collected',endLabel:'Net cash'},{key:'estimated',label:'Estimated booking value',endLabel:'Estimated',dashed:true}]} />
