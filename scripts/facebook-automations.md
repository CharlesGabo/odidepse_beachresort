...
- Reply rules: save classification, automatic reply, and comment alert preferences plus category templates. A collapsed advanced panel allows up to 40 comma- or line-separated English, Tagalog, or Taglish keywords per category without crowding the normal rules form. Complaint keywords take priority and General is the fallback. Automatic replies are enabled by default. Complaints receive only the configured acknowledgement and remain flagged for a person; a blank template skips the automatic response. Updates apply to new intake, not historical items.
...
The public callback is `/api/facebook-webhook.php`. Configure Meta app/Page credentials server-side, HTTPS verification and webhook signature checks, correct Page identity, subscriptions and permissions, duplicate-event handling, and lead-detail retrieval. The Cloudflare preview launcher runs the worker every five seconds while its window remains open. On production hosting, run `php scripts/facebook-worker.php` every minute from cron to resolve the sender's display name and deliver eligible automatic replies. If Meta does not return a name, the inbox retains the `Messenger guest` fallback. Lead events remain blocked until lead-detail retrieval is completed. The worker rechecks source, Page, conversation age, status, category, and template at dispatch; manual entries and old queued replies never become eligible just because the connection is enabled.
