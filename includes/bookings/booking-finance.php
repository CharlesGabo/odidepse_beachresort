...
{
    $query = $db->prepare('SELECT agreed_total, currency, finance_revision, status FROM bookings WHERE id = ?');
    ...
    return $booking + ['paid' => financeDecimal($balance['paid']), 'refunded' => financeDecimal($balance['refunded']),
        ...
        'balance' => $booking['agreed_total'] === null ? null : financeDecimal(financeCents($booking['agreed_total']) - $balance['net']),
        'entries' => $entries, 'audit' => $query->fetchAll()];
}

function financeRequireTotal(array $booking): void
{
    if ($booking['agreed_total'] === null) throw new InvalidArgumentException('Set the agreed total in booking Finance before confirming this request.');
}
